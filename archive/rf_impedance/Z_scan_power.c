...
*                                                                             *
* usage: $ ./Z_scan_grid <LO_freq> <bias> <LO_voltage>                        *
******************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    {   
        ...
        std::cerr << "\tusage: $ ./Z_scan_grid <LO_freq> <bias> <LO_voltage>\n";
        return 1;    
    }
    else
    {   
    ...
    std::cout << "# \n# Z.real | Z.imag |  DC current | LSB Gain | USB Gain | Noise\n";
    
    for (int i=1; i<=400; i++)
    {
        for (int j=1; j<=400; j++)
        {
            Rf.set(Complex(i,j)/100.0);
            mix.balance();
