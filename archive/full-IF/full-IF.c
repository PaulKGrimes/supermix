...
int main(int argc, char **argv)
{
    ...
    configfile = dotconf_create("full-IF.conf",
                        options, NULL, CASE_INSENSITIVE);
                        
    if (!configfile)
    {
        printf("Error opening config file \"full-IF.conf\".\n");
        return 1;
    }
    
    if (dotconf_command_loop(configfile) == 0)
    {
        printf("Error reading config file \"full-IF.conf\".\n");
    }
...
#include "mixer_build.inc"
    ...
    if (SCANNED_VARIABLE == "FREQUENCY")
    {
	    ...
	    {	If_Freq = get_If_Freq(scan_var);}
            ...
            if (TUNER_TYPE == "BELITSKY" || TUNER_TYPE == "BEL_WITH_RFCHOKE")
            {
                ...
                ZembUSB  = (1.0/((TwoPiI*(Frequency+If_Freq)/GHz*1.0e9*Sis_1.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_1.Rn;
                
                Zstub = ((1.0 + tuner_lsb_data.S[1][1])/(1.0-tuner_lsb_data.S[1][1]))*device::Z0;
                ZembLSB  = (1.0/((TwoPiI*(Frequency-If_Freq)/GHz*1.0e9*Sis_1.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_1.Rn;

                Zstub = ((1.0 + if_data.S[1][1])/(1.0-if_data.S[1][1]))*device::Z0;
                ZIfemb = (1.0/((TwoPiI*If_Freq/GHz*1.0e-9*Sis_1.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_1.Rn;
                ...
                ZembUSB2  = (1.0/((TwoPiI*(Frequency+If_Freq)/GHz*1.0e9*Sis_2.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_2.Rn;
                
                Zstub = ((1.0 + tuner_lsb_data.S[2][2])/(1.0-tuner_lsb_data.S[2][2]))*device::Z0;
                ZembLSB2  = (1.0/((TwoPiI*(Frequency-If_Freq)/GHz*1.0e9*Sis_2.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_2.Rn;

                Zstub = ((1.0 + if_data.S[2][2])/(1.0-if_data.S[2][2]))*device::Z0;
                ZIfemb2 = (1.0/((TwoPiI*If_Freq/GHz*1.0e-9*Sis_2.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_2.Rn;
	...
	// Get the tuner data so we can calculate the embedding impedance
        ...
        if (TUNER_TYPE == "BELITSKY" || TUNER_TYPE == "BEL_WITH_RFCHOKE")
        {
            printf("# %s (%s)| LSB Gain | USB Gain | IF Output 0K (K)| IF Output 77K (K)| IF Output 297K (K)| DC Current (junc 1) (uA)| DC Current (junc 2) (uA)| IF Embedding Impedance (junc 1) | IF Embedding Impedance (junc2)\n"
                , SCANNED_VARIABLE.c_str(), SCAN_unitStr.c_str());
        }
        else
        {
            printf("# %s (%s)| LSB Gain | USB Gain | IF Output 0K (K)| IF Output 77K (K)| IF Output 297K (K)| DC Current (uA)| IF Embedding Impedance\n"
                , SCANNED_VARIABLE.c_str(), SCAN_unitStr.c_str());
        }
	    ...
	    if_data = If.get_data();
            ...
            if (TUNER_TYPE == "BELITSKY" || TUNER_TYPE == "BEL_WITH_RFCHOKE")
            {
                ...
                printf("%f      %f      %f      %f      %f      %f      %f      %f      %f %fi      %f %fi\n",
                    s,
                    abs(result.S[IF_PORT][LSB_PORT]),
                    abs(result.S[IF_PORT][USB_PORT]),
                    term_result.C[IF_PORT][IF_PORT].real/Kelvin,
    ...
    else // We are scanning BIAS or LO_POWER
    {
        ...
        if (TUNER_TYPE == "BELITSKY" || TUNER_TYPE == "BEL_WITH_RFCHOKE")
        {
            ...
            Zstub = ((1.0 + if_data.S[2][2])/(1.0-if_data.S[2][2]))*device::Z0;
            ZIfemb2 = (1.0/((TwoPiI*If_Freq/GHz*1.0e-9*Sis_1.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_1.Rn;
