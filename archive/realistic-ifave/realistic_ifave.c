/*******************************************************************************
** realistic.c - test of using dotconf library to control SuperMix parameters **
**               at run time.                                                 **
** ========================================================================== **
** Author - Paul Grimes                                                       **
** Date   - 14/07/03                                                          **
...
** 01/09/03 - altered from simple.c to realistic.c                            **
*******************************************************************************/
...
int main(int argc, char **argv)
{
    ...
    configfile = dotconf_create("realistic.conf",
                        options, NULL, CASE_INSENSITIVE);

    if (!configfile)
    {
        printf("Error opening config file \"realistic.conf\".\n");
        return 1;
    }

    if (dotconf_command_loop(configfile) == 0)
    {
        printf("Error reading config file \"realistic.conf\".\n");
    }

    dotconf_cleanup(configfile);
    // Finished with simple.conf

    printf("# Finished with realistic.conf\n");
    ...
    printf("# Finished reading config files\n");
...
#include "mixer_build.inc"
    ...
    if (SCANNED_VARIABLE == "FREQUENCY")
    {
	    ...
	    if_data = If.get_data();
            ...
            Load_Temp = 77.0*Kelvin;
            cold_result = mix.get_term_data();
            Load_Temp = 297.0*Kelvin;
	...
	//printf("# Got RF data\n");
	    ...
	    ZIfemb = (1.0/((TwoPiI*If_Freq/GHz*1.0e-9*Sis_1.Cap/fFarad*1.0e-15)+(1.0/Zstub)))/Sis_1.Rn;
            ...
            printf("# Junction LO Embedding Impedance : %f+%fi\n", Zemb.real/1.0, Zemb.imaginary/1.0);
