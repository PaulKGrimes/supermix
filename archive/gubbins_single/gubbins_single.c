...
int main(int argc, char **argv)
{
    ...
    configfile = dotconf_create("gubbins_single.conf",
                        options, NULL, CASE_INSENSITIVE);
                        
    if (!configfile)
    {
        printf("Error opening config file \"gubbins_single.conf\".\n");
        return 1;
    }
    
    if (dotconf_command_loop(configfile) == 0)
    {
        printf("Error reading config file \"gubbins_single.conf\".\n");
    }
...
#include "mixer_build.inc"
    ...
    if (SCANNED_VARIABLE == "FREQUENCY")
    {
        ...
        {
            printf("# %s (%s)| LSB Gain | USB Gain | IF Output 0K (K)| IF Output 77K (K)| IF Output 297K (K)| DC Current (junc 1) (uA)| DC Current (junc 2) (uA)| LO Emb Z | IF Emb Z | LSB Emb Z | USB Emb Z \n"
                , SCANNED_VARIABLE.c_str(), SCAN_unitStr.c_str());
        }
        else
        {
            printf("# %s (%s)| LSB Gain | USB Gain | IF Output 0K (K)| IF Output 77K (K)| IF Output 297K (K)| DC Current (uA)|  LO Emb Z | IF Emb Z | LSB Emb Z | USB Emb Z \n"
                , SCANNED_VARIABLE.c_str(), SCAN_unitStr.c_str());
        }
