...
BesselFn::BesselFn()
{
    DOUBLE_PRECISION = std::numeric_limits<double>::epsilon();
}
 ...
 */
double BesselFn::Jnx(int n, double x)
{
  ...
  if(n>=0)
    ...
    return pow(-1.0,(abs_n%2)+2)*J[abs_n];
}
 ...
 * and normalised using J0(x) + 2J1(x) + 2J2(x) +...+2Jn(x) = 1;
 */
bool BesselFn::setArrayJnx(unsigned int n_last, double x, real_vector &J)
{
  if(n_last <= 0)
    return false;
    
  // init
  J.resize(n_last+1); // order 0th to n_last
  J.reindex(Index_C);
  J.clean();
  ...
  J.resize(n_last+1); // Resize to just keep those values we want.
 ...
 * Very time consuming
 */
double BesselFn::IntegratedJnx(int n, double x)
{
    int NN = 1000;
    double dphi = M_PI/NN;
    double phi = dphi/2.0;
