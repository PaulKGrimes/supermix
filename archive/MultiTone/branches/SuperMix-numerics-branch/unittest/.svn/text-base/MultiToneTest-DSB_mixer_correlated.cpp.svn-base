/* MultiToneTest-DSB_mixer_correllated.cpp
*
...
int main(int argc, char* argv[])
{
    ...
    if (argc != 6)
    {
        cerr << "usage: ./MultiToneTest-DSB_sat <LO Freq> <IF Freq> <Bias> <LO Voltage> <SF Voltage>\n";
        return 1;        
    } 
    else
    {
    ...
    for (int phi=0; phi <360; phi+=6)
    {
        ...
        for (int phi2=0; phi2 <360; phi2+=6)
        {
            ...
            Complex phasor = Complex(cos((phi-phi2/2.0)/180*Pi), sin((phi-phi2/2.0)/180*Pi));
            Complex phasor2 = Complex(cos((phi+phi2/2.0)/180*Pi), sin((phi+phi2/2.0)/180*Pi));
