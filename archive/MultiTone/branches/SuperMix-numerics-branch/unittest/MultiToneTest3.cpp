...
int main()
{
    ...
    mt.set_Zt(1, 0, Complex(0.0, 0.0));
    
    mt.add_harmonic(1);
    mt.set_Vs(1, 1, Complex(0.0, 0.0));
    mt.set_Zt(1, 1, Complex(0.0, 0.0));
    
    mt.add_harmonic(1);
    mt.set_Vs(1, 2, Complex(0.0, 0.0));
    mt.set_Zt(1, 2, Complex(0.0, 0.0));
    
    cout << "# IF frequency set up\n";
   
    // Set up signal frequency
    mt.add_frequency(mt.get_freq(0,0) + mt.get_freq(1,0));
    mt.set_Vs(2, 0, Complex(1e-3, 0.0));
    ...
    cout << "# Signal Volts | KK | DC Current | Pump Current | IF Current | IF Harmonic 1 | IF Harmonic 2 | Signal Current | Image Current | Iterations\n";
    ...
    for (int v=0; v<=80; v++)
    {
        ...
        mt.balance();
        cout << "\t" << real(mt.get_I(0.0)) << "\t" << abs(mt.get_I(0,0)) << "\t" << abs(mt.get_I(1,0)) << "\t" << abs(mt.get_I(1,1)) << "\t" << abs(mt.get_I(1,2)) << "\t" << abs(mt.get_I(2,0)) << "\t" << abs(mt.get_I(3,0)) << "\t" << mt.get_iterations() << endl;
    }
