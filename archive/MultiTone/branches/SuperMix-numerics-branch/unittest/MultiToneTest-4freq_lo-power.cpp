...
int main()
{
    ...
    cout << "# LO Volts | Unpumped DC | KK | DC Current | Pump Current | IF Current | Signal Current | Image Current | Iterations\n";
    
    for (int v=0; v<=100; v++)
    {
        long double V = v/20.0; 
        cout << V;
        mt.set_Vs(0, 0, Complex(V));
        mt.balance();
        cout << "\t" << mt.get_unpumped_Idc(0.9);
        cout << "\t" << mt.get_unpumped_Ikk(0.9);
